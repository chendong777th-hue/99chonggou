...
import 'package:tencent_cloud_chat_uikit/business_logic/view_models/tui_chat_global_model.dart'
    show ConvType, HistoryMessagePosition, TUIChatGlobalModel;
...
bool _conversationUiIsDark(TUITheme theme) {
...
Color _conversationPageBackground(TUITheme theme) {
  if (!_conversationUiIsDark(theme)) {
...
Color _conversationItemBackground(
  TUITheme theme, {
  ...
  final itemColor = theme.conversationItemBgColor;
  if (itemColor != null &&
      ThemeData.estimateBrightnessForColor(itemColor) == Brightness.dark) {
    return itemColor;
  }
  return _conversationPageBackground(theme);
}

final ValueNotifier<bool> conversationEditingNotifier =
    ValueNotifier<bool>(false);
final ValueNotifier<bool> conversationAllSelectedNotifier =
    ValueNotifier<bool>(false);
  ...
  static void _executeFullRefresh() {
    ...
    ConversationPinFlickerLog.log(
      ...
      extras: <String, Object?>{
        ...
        'deferring':
            ConversationListNotifier.instance.isDeferringPinReorder,
      },
    );
...
class _ConversationState extends State<Conversation> {
  ...
  void initState() {
    ...
    final secondaryInitDelay = kIsWeb
        ? const Duration(milliseconds: 1200)
        : const Duration(milliseconds: 1500);
    Future.delayed(secondaryInitDelay, () {
      if (mounted) {
        unawaited(
          GroupJoinApplicationService.instance.refresh(syncMembership: false),
        );
        unawaited(GroupSystemNoticeService.instance.refresh(force: true));
      }
    });
    if (kIsWeb) {
    ...
    if (kIsWeb) {
      Future<void>.delayed(const Duration(seconds: 2), () {
        ...
        PlatformOfficialAccountService.loadDismissedState(force: true)
            .then((_) async {
    ...
    PlatformOfficialAccountService.infoRevision
        .addListener(_onOfficialInfoUpdated);
    ConversationRefreshBus.instance.revision
        .addListener(_onConversationRefreshRequested);
    ...
    ConversationListNotifier.instance
        .addListener(_invalidateVisibleConversationsCache);
    archivedConversationC2cIDsNotifier
        .addListener(_invalidateVisibleConversationsCache);
    archivedConversationGroupIDsNotifier
        .addListener(_invalidateVisibleConversationsCache);
    GroupMembershipSyncService.instance.joinedGroupsRevision
        .addListener(_invalidateVisibleConversationsCache);
    ...
    unawaited(() async {
      try {
        ...
        for (final folder in ConversationFolderStore.instance.folders) {
          ...
          next[folder.folderId] = await ConversationLocalStore.instance
              .sumUnreadForConversationIds(ids);
        }
        if (!mounted) {
          return;
        }
  ...
  void _queuePatchConversationUpdates({String? reason}) {
    ...
    _pendingPatchConversationIds
        .addAll(_collectPatchConversationIds(reason: reason));
    _patchConversationTimer?.cancel();
  ...
  void _scheduleFeedScrollRestore(
    double offset, {
  ...
  void _handleFeedScrollingState(bool scrolling) {
    if (scrolling && !_feedWasScrolling) {
      ...
      ConversationHistoryWarmScheduler.instance
          .cancelViewportWarm(reason: 'scroll_start');
  ...
  void _scheduleViewportWarmAfterSettle({required String reason}) {
    if (!ConversationHistoryWarmScheduler.viewportWarmEnabled) {
      return;
    }
    _viewportWarmSettleTimer?.cancel();
  ...
  void _runViewportWarmNow({required String reason}) {
    if (!mounted || !ConversationHistoryWarmScheduler.viewportWarmEnabled) {
      return;
    }
    if (!_feedScrollController.hasClients) {
      return;
    ...
    final archived = _getArchivedConversations(_archivedIDsNotifier.value);
    final rows = buildConversationFeedRows(
      conversations: visible,
      includeArchivedEntry:
          _selectedFolderId == null && archived.isNotEmpty,
  ...
  void _captureFeedScrollAnchor() {
    ...
    final rawIndex = (pixels / _feedRowEstimateHeight).floor();
    ...
    _pendingFeedScrollAnchorIntra =
        pixels - index * _feedRowEstimateHeight;
  ...
  void _publishViewportAnchorFromScroll() {
    ...
    final pixels = _feedScrollController.offset;
    final mid =
        (pixels + _feedScrollController.position.viewportDimension * 0.5) /
            _feedRowEstimateHeight;
    final index = mid.floor().clamp(0, visible.length - 1);
    final id = visible[index].conversationID.trim();
    if (id.isNotEmpty) {
      ConversationListNotifier.instance.updateViewportAnchor(id);
    }
  ...
  void _restoreFeedScrollAnchor() {
    ...
    for (var i = 0; i < visible.length; i++) {
      if (visible[i].conversationID.trim() == anchorId) {
        newIndex = i;
        break;
      }
    }
    if (newIndex < 0) {
      // 锚点被裁：找最近仍在窗内的邻居（按原列表顺序退化到首个仍在的）。
      // 找不到则 no-op，禁止 jumpTo(0)。
      return;
    }
    final position = _feedScrollController.position;
    final target =
        (newIndex * _feedRowEstimateHeight + intra)
            .clamp(0.0, position.maxScrollExtent);
    if ((target - position.pixels).abs() < 0.5) {
      return;
    }
    _feedScrollController.jumpTo(target);
    _lastKnownFeedScrollOffset = target;
  }
  ...
  Future<void> _maybeFillFeedViewportOnce() async {
    ...
    while (mounted &&
        !_viewportFillDone &&
        _viewportFillPagesDone < maxPages) {
      if (!_feedScrollController.hasClients) {
        return;
      }
      if (_feedScrollController.position.maxScrollExtent > 0) {
  ...
  void dispose() {
    ...
    PlatformOfficialAccountService.infoRevision
        .removeListener(_onOfficialInfoUpdated);
    ConversationRefreshBus.instance.revision
        .removeListener(_onConversationRefreshRequested);
    ConversationListSyncNotifier.instance
        .removeListener(_onFeedSyncStateChanged);
    ConversationListNotifier.instance
        .removeListener(_scheduleFolderUnreadRefresh);
    ConversationListNotifier.instance
        .removeListener(_invalidateVisibleConversationsCache);
    archivedConversationC2cIDsNotifier
        .removeListener(_invalidateVisibleConversationsCache);
    archivedConversationGroupIDsNotifier
        .removeListener(_invalidateVisibleConversationsCache);
    GroupMembershipSyncService.instance.joinedGroupsRevision
        .removeListener(_invalidateVisibleConversationsCache);
    ConversationFolderStore.instance.foldersNotifier
        .removeListener(_onFoldersChanged);
  ...
  String? _chatCacheConversationKey(V2TimConversation conversation) {
    ...
    final conversationID =
        TencentUtils.checkString(conversation.conversationID);
  ...
  ConvType _chatCacheConversationType(V2TimConversation conversation) {
    return conversation.type == 2 ? ConvType.group : ConvType.c2c;
  }

  static const Duration _chatOpenUrlResolveBudget = Duration(milliseconds: 220);
  static const Duration _chatOpenWarmBudget = Duration(milliseconds: 180);

  void _injectBootstrapMessages(
    TUIChatGlobalModel globalModel,
    String conversationKey,
    List<V2TimMessage> messages, {
    bool? mayHaveOlder,
    String source = 'bootstrap_inject',
  }) {
    if (messages.isEmpty) {
      return;
    }
    final existing = globalModel.messageListMap[conversationKey];
    final existingNewest = existing == null || existing.isEmpty
        ? 0
        : (existing.first.timestamp ?? 0);
    final fetchedNewest =
        messages.isEmpty ? 0 : (messages.first.timestamp ?? 0);
    // late / warm inject：禁止用明显更旧的归档窗盖掉当前首屏。
    if (existing != null &&
        existing.isNotEmpty &&
        HistoryPaginationAnchor.isStaleArchiveDominatedWindow(
          messages,
          referenceTimestampSec: existingNewest,
        )) {
      ChatDiagLog.log(
        'ChatHistory',
        '${source}_skip_stale_archive',
        conversationID: conversationKey,
        extras: <String, Object?>{
          'existingCount': existing.length,
          'fetchedCount': messages.length,
          'existingNewestTs': existingNewest,
          'fetchedNewestTs': fetchedNewest,
        },
      );
      return;
    }
    ...
    if (completeWindow || exhaustedOlder) {
      globalModel.markInitialHistoryLoaded(conversationKey);
    }
    globalModel.markInitialHistoryMayHaveOlder(
      conversationKey,
      // 首轮不足窗口且 SDK 仍可能有更早消息时继续探测；明确无更早则关掉。
      mayHaveOlder: mayHaveOlder ?? true,
    );
  }

  Future<void> _bootstrapChatMessagesBeforeOpen(
    V2TimConversation conversation, {
    bool awaitHydrate = true,
  }) async {
    ...
    // 重连预热已满窗：不再 await peek / URL resolve，先进页再后台补图。
    if (ConversationPreviewHistorySync.isWarmWindowReadyForOpen(
      ...
      final cached = globalModel.rawMessageList(conversationKey) ??
          globalModel.messageListMap[conversationKey];
      if (cached != null && cached.isNotEmpty) {
        ChatImageMessagePrefetch.fromMessages(cached);
        unawaited(
          ChatImageMessagePrefetch.resolveOnlineUrlsForMessages(
            cached,
            budget: _chatOpenUrlResolveBudget,
          ),
        );
      }
      return;
    ...
    // in-flight 必须覆盖到 inject 完成，避免页内 gate 在空窗上误开二次 peek。
    final hydrateTask = _runColdOpenHydrate(
      conversation: conversation,
      conversationKey: conversationKey,
      globalModel: globalModel,
    );
    globalModel.registerOpenHydrateInFlight(conversationKey, hydrateTask);
    if (awaitHydrate) {
      await hydrateTask;
    }
  }

  Future<void> _runColdOpenHydrate({
    required V2TimConversation conversation,
    required String conversationKey,
    required TUIChatGlobalModel globalModel,
  }) async {
    ChatOpenPerfLog.mark(
      'bootstrap_peek_start',
      conversationID: conversationKey,
    );
    final prefetch = await ChatImageMessagePrefetch.prepareBeforeChatOpen(
      conversation,
      warmBudget: _chatOpenWarmBudget,
      onLatePrepared: (lateResult) {
        _injectBootstrapMessages(
          globalModel,
          conversationKey,
          lateResult.messages,
          mayHaveOlder: lateResult.hasMoreOlder,
          source: 'bootstrap_late_inject',
        );
        SilentArchiveService.instance.scheduleAfterBootstrapInject(
          conversation: conversation,
          conversationKey: conversationKey,
          injectedCount: lateResult.messages.length,
          requestedCount: HistoryMessageDartConstant.initialOpenFetchCount,
        );
      },
    );
    ChatOpenPerfLog.mark(
      ...
      conversationID: conversationKey,
      extras: <String, Object?>{
        'fetched': prefetch.messages.length,
        'hasMoreOlder': prefetch.hasMoreOlder,
      },
    );
    _injectBootstrapMessages(
      globalModel,
      conversationKey,
      prefetch.messages,
      mayHaveOlder: prefetch.hasMoreOlder,
      source: 'bootstrap_warm_inject',
    );
    SilentArchiveService.instance.scheduleAfterBootstrapInject(
      conversation: conversation,
      conversationKey: conversationKey,
      injectedCount: prefetch.messages.length,
      requestedCount: HistoryMessageDartConstant.initialOpenFetchCount,
    );
    if (prefetch.messages.isEmpty) {
      return;
    }
    // URL 补全不挡 in-flight；页内可先出消息再补图。
    final urlSw = Stopwatch()..start();
    unawaited(
      ChatImageMessagePrefetch.resolveOnlineUrlsForMessages(
        prefetch.messages,
        budget: _chatOpenUrlResolveBudget,
      ).then((_) {
        ChatImageMessagePrefetch.fromMessages(prefetch.messages);
        ChatOpenPerfLog.mark(
          'bootstrap_url_resolve_done',
          conversationID: conversationKey,
          extras: <String, Object?>{'urlResolveMs': urlSw.elapsedMilliseconds},
        );
      }),
    );
  }
  ...
  void _handleOnConvItemTaped(V2TimConversation? selectedConv) async {
    ...
    final openCacheKey = _chatCacheConversationKey(selectedConv) ?? conversationID;
    ...
    ChatOpenPerfLog.beginOpen(
      ...
      extras: <String, Object?>{
        ...
        'initialLoaded':
            serviceLocator<TUIChatGlobalModel>().hasInitialHistoryLoaded(
          openCacheKey,
        ),
      },
    );

    // 暖/冷均先 await 灌窗，再开转场，使滑入时已是有消息的完整页。
    ChatOpenPerfLog.mark('bootstrap_before_await');
    final warmReadyForOpen =
    ...
    try {
      await _bootstrapChatMessagesBeforeOpen(selectedConv).timeout(
        const Duration(milliseconds: 1000),
        onTimeout: () {
          ChatOpenPerfLog.mark(
            'cold_bootstrap_timeout_before_push',
            conversationID: openCacheKey,
            extras: <String, Object?>{
              'rawCount': serviceLocator<TUIChatGlobalModel>().rawMessageCount(
                openCacheKey,
              ),
              'wasWarmReady': warmReadyForOpen,
            },
          );
        },
      );
    } catch (_) {
      // hydrate 失败仍继续 push，页内 gate 兜底。
    }
    ChatOpenPerfLog.mark(
      'bootstrap_after_await',
      extras: <String, Object?>{
        'mode': warmReadyForOpen ? 'warm_await' : 'cold_await',
        'rawCount': serviceLocator<TUIChatGlobalModel>().rawMessageCount(
          openCacheKey,
        ),
        'initialLoaded':
  ...
  Future<void> _deleteConversation(V2TimConversation conversation) async {
    ...
    ConversationRefreshBus.instance
        .requestRefresh(reason: 'conversation_deleted');
    if (mounted) {
      setState(() {});
    }
  }
  ...
  Future<void> _toggleConversationDisturb(
    ...
    final isDisturb = conversation.recvOpt != 0;
    ...
    final result = groupID.isNotEmpty
        ? await _messageService.setGroupReceiveMessageOpt(
            groupID: groupID,
            opt: targetOpt,
          )
        : userID.isEmpty
    ...
    if (result != null && result.code == 0) {
      // 列表行有指纹缓存：必须走 Notifier，否则 UI 不刷免打扰图标。
      ConversationListNotifier.instance.applyRecvOptLocally(
        conversationID: conversation.conversationID,
        recvOpt: targetOpt.index,
      );
      conversation.recvOpt = targetOpt.index;
  ...
  void _syncEditingStateNotifiers() {
    ...
    conversationAllSelectedNotifier.value =
        _isAllSelected(_getVisibleConversations());
  }
  ...
  bool _isConversationArchivedInEitherScope(String conversationID) {
    return conversationIdInArchivedSet(
          archivedConversationIDsNotifierFor(ConversationArchiveScope.c2c)
              .value,
          conversationID,
        ) ||
        conversationIdInArchivedSet(
          archivedConversationIDsNotifierFor(ConversationArchiveScope.group)
              .value,
          conversationID,
        );
  }
  ...
  List<V2TimConversation> _getVisibleConversations() {
    ...
    final typed = ConversationListNotifier.instance.conversations;
    ...
    final filtered = typed
        .where((conversation) {
          ...
          if (PlatformOfficialAccountService.shouldHideConversation(
            conversation,
          )) {
            return false;
          }
    ...
    final injected =
        PlatformOfficialAccountService.conversationsIfMissingFrom(filtered);
    if (injected.isEmpty) {
      _cachedVisibleConversations = filtered;
      return filtered;
    }
  ...
  bool _ensureFolderNameAvailable(
    String name, {
  ...
  Future<void> _showAddToFolderSheet(V2TimConversation conversation) async {
    ...
    final currentFolder = ConversationFolderStore.instance
        .folderContaining(conversation.conversationID);
    final action = await AppDialog.actionSheet<String>(
      title: AppI18n.of(context).t(
        zhHans: '添加到分组',
        zhHant: '添加到分組',
    ...
    if (action == '__create__') {
      ...
      try {
        final folder =
            await ConversationFolderSyncService.instance.createFolder(
          name: name,
        );
  ...
  void toggleSelectAll() {
    ...
    if (_isAllSelected(visibleConversations)) {
      ...
      _selectedConversationIDs = visibleConversations
  ...
  bool _isAllSelected(List<V2TimConversation> conversations) {
    return conversations.isNotEmpty &&
        conversations.every(
          (conversation) => _selectedConversationIDs.contains(
            conversation.conversationID,
          ),
        );
  }
  ...
  Future<void> _markSelectedAsRead() async {
    ...
    final selectedConversations =
        _getSelectedConversations(visibleConversations);
    final selectedIds = Set<String>.from(_selectedConversationIDs);
    // 非全选时只处理有未读的会话：避免勾选几百个却发起几百次 SDK clean。
    final unreadSelected = selectedConversations
        .where((conversation) => (conversation.unreadCount ?? 0) > 0)
        .toList();
    final totalUnreadSum = unreadSelected.fold<int>(
      0,
      (sum, c) => sum + (c.unreadCount ?? 0),
    );
    MarkSelectedReadLog.log('mark_selected_start', {
      'scope': widget.listScope.name,
      'visibleCount': visibleConversations.length,
      'selectedCount': selectedConversations.length,
      'unreadSelectedCount': unreadSelected.length,
      'unreadSum': totalUnreadSum,
      'allSelected': _isAllSelected(visibleConversations),
      'selectedIds': MarkSelectedReadLog.summarizeIds(selectedIds),
      'unreadDetail': MarkSelectedReadLog.summarizeUnread(unreadSelected),
    });
    ...
    for (final conversation in unreadSelected) {
      MarkSelectedReadLog.log('mark_selected_local_clear', {
        'conv': conversation.conversationID,
        'unreadBefore': conversation.unreadCount ?? 0,
      });
      await ConversationUnreadClearService.clearLocalForOpen(
        conversation: conversation,
        markViewModelReadLocally: _controller.model.markConversationReadLocally,
    ...
    final bulk = ConversationUnreadClearService.shouldUseTypeBulkClean(
      visible: visibleConversations,
      selectedIds: selectedIds,
    );
    if (bulk != null) {
      MarkSelectedReadLog.log('mark_selected_decision_bulk', {
        'scope': widget.listScope.name,
        'isGroup': bulk.isGroup,
        'typeId': bulk.isGroup
            ? ConversationUnreadClearService.sdkCleanTypeGroup
            : ConversationUnreadClearService.sdkCleanTypeC2c,
        'unreadSelectedCount': unreadSelected.length,
      });
      unawaited(
        ConversationUnreadClearService.cleanSdkUnreadForType(
          isGroup: bulk.isGroup,
          markViewModelReadLocally:
              _controller.model.markConversationReadLocally,
        ),
      );
    } else {
      MarkSelectedReadLog.log('mark_selected_decision_queue', {
        'scope': widget.listScope.name,
        'queueSize': unreadSelected.length,
        'intervalMs': ConversationUnreadClearService
            .queuedSdkCleanInterval.inMilliseconds,
        'queueIds': MarkSelectedReadLog.summarizeIds(
          unreadSelected.map((c) => c.conversationID),
        ),
      });
      for (final conversation in unreadSelected) {
        ...
        unawaited(
          ConversationUnreadClearService.enqueueSdkUnreadClean(
            id,
            hadUnread: true,
          ),
        );
  ...
  Future<void> _archiveSelected() async {
    final selectedConversations =
        _getSelectedConversations(_getVisibleConversations());
    ...
    _syncEditingStateNotifiers();
    ConversationRefreshBus.instance
        .requestRefresh(reason: 'conversation_archive');
  }
  ...
  Future<void> _deleteSelected() async {
    ...
    final confirmed =
        await _confirmDeleteConversation(count: selectedConversations.length);
    if (confirmed != true) return;
    ...
    _syncEditingStateNotifiers();
    ConversationRefreshBus.instance
        .requestRefresh(reason: 'conversation_delete_selected');
  }
  ...
  String _buildConversationPreviewSubtitle(V2TimConversation conversation) {
    ...
    final groupTipPreview =
        GroupTipsMessageHelper.messagePreviewAbstract(lastMessage);
  ...
  Widget _buildCachedConversationFeed(
    BuildContext context,
    TUITheme theme,
  ) {
    if (ConversationListNotifier.instance.hasLocalData) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final titleWidget = buildOfficialAccountConversationNickName(
          userId: conversation.userID,
          name: item.title,
          titleColor: theme.conversationItemTitleTextColor,
        );
        return KeyedSubtree(
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          AppUserAvatar(
            key: ValueKey<String>(
              'conversation_avatar_${conversation.conversationID}',
            ),
            faceUrl: _conversationFaceUrl(conversation),
            showName: _conversationShowName(conversation),
            size: 56,
          ),
  ...
  Widget _buildConversationRow(
    ...
    final displayedOnlineStatus = localSetting.isShowOnlineStatus &&
        ...
        ? (PlatformOfficialAccountService.showsVerifiedBadge(
                conversationItem.userID)
            ? V2TimUserStatus(
                userID: conversationItem.userID,
                statusType: 1,
              )
    ...
    Widget conversationLineItem() {
      ...
      return AnimatedContainer(
        ...
        // 对齐 Telegram 短反馈；重排 delay 约 180ms。
        ...
        child: AppListPressable(
          ...
          child: TIMUIKitConversationItem(
            ...
            lastMessageAbstractBuilder: conversationListLastMessageAbstract,
            faceUrl: _conversationFaceUrl(conversationItem),
            avatarWidget: _conversationAvatarWidget(
              conversationItem,
              onlineStatus: displayedOnlineStatus,
            ),
            nickName: _conversationShowName(conversationItem),
            nickNameWidget: buildOfficialAccountConversationNickName(
              userId: conversationItem.userID,
              name: _conversationShowName(conversationItem),
              titleColor: theme.conversationItemTitleTextColor,
            ),
            avatarBorderRadius: BorderRadius.circular(999),
            avatarSize: 56,
            isDisturb: (conversationItem.groupType == "Meeting"
                ? false
            ...
            customEmojiStickerList: _conversationPreviewEmojiList(context),
            onlineStatus: displayedOnlineStatus,
    ...
    final endSlideActions = _itemEndSlidableBuilder(conversationItem);
    ...
    return conversationSlidable(
      ...
      startActionPane: startSlideActions == null
          ? null
          : conversationActionPane(
              webFeel: webFeel,
  ...
  List<ConversationItemSlidePanel> _itemEndSlidableBuilder(
    ...
    final isDisturb = conversationItem.recvOpt != 0;
          ...
          Expanded(
            child: GestureDetector(
              ...
              child: TIMUIKitConversationItem(
                ...
                avatarBorderRadius: BorderRadius.circular(999),
                avatarSize: 56,
                isDisturb: (conversation.groupType == "Meeting"
                    ? false
          ...
          children: [
            ...
            if (_hostsDesktopLoginBanner)
              ValueListenableBuilder<List<UserDevice>>(
                valueListenable:
                    DesktopLoginSessionService.instance.devices,
...
class _ArchivedConversationPageState extends State<ArchivedConversationPage> {
  ...
  Future<List<V2TimConversation>> _hydrateMissingFromSdk(int budget) async {
    ...
    for (final id in batch) {
      try {
        final conversation =
            await _conversationService.getConversation(conversationID: id);
  ...
  Future<void> _reloadArchiveFirstPage() async {
    ...
    try {
      ...
      var page = await ConversationLocalStore.instance.loadOlderAmongIds(
      ...
      if (page.length < pageSize && _missingArchivedIds.isNotEmpty) {
        final hydrated = await _hydrateMissingFromSdk(pageSize - page.length);
      ...
      setState(() {
        ...
        _hasMore = page.length >= pageSize || _missingArchivedIds.isNotEmpty;
  ...
  Future<void> _loadMoreArchived() async {
    ...
    try {
      ...
      if (page.isEmpty && _missingArchivedIds.isNotEmpty) {
        page = await _hydrateMissingFromSdk(pageSize);
        page.sort(ConversationLocalStore.compareConversationsForUi);
      ...
      setState(() {
        ...
        _hasMore = added > 0 &&
            (page.length >= pageSize || _missingArchivedIds.isNotEmpty);
        if (added == 0 && _missingArchivedIds.isEmpty) {
          _hasMore = false;
        }
        _loadingMore = false;
      });
  ...
  Future<void> _deleteArchivedConversation(
    ...
    ConversationRefreshBus.instance
        .requestRefresh(reason: 'conversation_archive_deleted');
    if (mounted) {
      setState(() {});
    }
  }
  ...
  Future<void> _toggleArchivedConversationDisturb(
    ...
    final result = groupID.isNotEmpty
        ? await messageService.setGroupReceiveMessageOpt(
            groupID: groupID,
            opt: targetOpt,
          )
        : userID.isEmpty
  ...
  Future<void> _updateArchivedStatusForConversations(
    ...
    if (!archived) {
      final removeIds = scopedConversations
      ...
      _displayedIds.removeWhere(
        (id) => removeIds.any(
          (r) => MessageConversationId.sameConversation(id, r),
        ),
      );
    }
    if (mounted) {
  ...
  bool _isAllSelected(List<V2TimConversation> archivedConversations) {
    return archivedConversations.isNotEmpty &&
        archivedConversations.every(
          (conversation) => _selectedConversationIDs.contains(
            conversation.conversationID,
          ),
        );
  ...
  void _toggleSelectAll(List<V2TimConversation> archivedConversations) {
    setState(() {
      if (_isAllSelected(archivedConversations)) {
        _selectedConversationIDs.clear();
      } else {
        _selectedConversationIDs = archivedConversations
            .map((conversation) => conversation.conversationID)
            .toSet();
      }
    });
  ...
  Future<void> _markSelectedAsRead(
    ...
    final selectedConversations =
        _getSelectedConversations(archivedConversations);
    final selectedIds = Set<String>.from(_selectedConversationIDs);
    // 非全选时只处理有未读的会话：避免勾选几百个却发起几百次 SDK clean。
    final unreadSelected = selectedConversations
        .where((conversation) => (conversation.unreadCount ?? 0) > 0)
    ...
    MarkSelectedReadLog.log('mark_selected_archive_start', {
      'scope': widget.listScope.name,
      'visibleCount': archivedConversations.length,
      'selectedCount': selectedConversations.length,
      'unreadSelectedCount': unreadSelected.length,
      'unreadSum': totalUnreadSum,
      'allSelected': _isAllSelected(archivedConversations),
      'selectedIds': MarkSelectedReadLog.summarizeIds(selectedIds),
      'unreadDetail': MarkSelectedReadLog.summarizeUnread(unreadSelected),
    });
    if (unreadSelected.isEmpty) {
    ...
    for (final conversation in unreadSelected) {
      MarkSelectedReadLog.log('mark_selected_archive_local_clear', {
        'conv': conversation.conversationID,
        'unreadBefore': conversation.unreadCount ?? 0,
      });
      await ConversationUnreadClearService.clearLocalForOpen(
    ...
    final bulk = ConversationUnreadClearService.shouldUseTypeBulkClean(
      visible: archivedConversations,
      selectedIds: selectedIds,
    );
    if (bulk != null) {
      MarkSelectedReadLog.log('mark_selected_archive_decision_bulk', {
        'scope': widget.listScope.name,
        'isGroup': bulk.isGroup,
        'typeId': bulk.isGroup
            ? ConversationUnreadClearService.sdkCleanTypeGroup
      ...
      MarkSelectedReadLog.log('mark_selected_archive_decision_queue', {
        'scope': widget.listScope.name,
        'queueSize': unreadSelected.length,
        'intervalMs': ConversationUnreadClearService
            .queuedSdkCleanInterval.inMilliseconds,
  ...
  Future<void> _unarchiveSelected(
    ...
    ConversationRefreshBus.instance
        .requestRefresh(reason: 'conversation_unarchive');
  }
  ...
  Future<void> _deleteSelected(
    ...
    // 保留归档标记：删除只清本端记录与会话实体，会话再次活跃时仍回归档。
    ConversationRefreshBus.instance
        .requestRefresh(reason: 'conversation_archive_delete_selected');
    if (mounted) {
      setState(() {
        _selectedConversationIDs.clear();
        _isEditing = false;
    ...
    final page = Scaffold(
        ...
        body: AnimatedBuilder(
          ...
          builder: (context, child) {
            final archivedConversations = _archiveItems;
                    ...
                    children: [
                      ...
                      Expanded(
                        child: AppListPressable(
                          ...
                          onLongPress: () =>
                              _showConversationPeek(conversation),
                          child: TIMUIKitConversationItem(
                            isCurrent: false,
                            conversationID: conversation.conversationID,
                            lastMessageAbstractBuilder:
                            ...
                            nickName: _conversationShowName(conversation),
                            nickNameWidget:
                                buildOfficialAccountConversationNickName(
                              userId: conversation.userID,
                              name: _conversationShowName(conversation),
                              titleColor: theme.conversationItemTitleTextColor,
                            ),
