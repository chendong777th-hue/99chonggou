                              ...
                              children: [
                                ...
                                Selector2<ChatUiStateStore,
                                  ...
                                  builder: (context, value, child) {
                                    ...
                                    final input = widget.textFieldBuilder !=
                                            null
                                    ...
                                    return widget.textFieldWrapperBuilder
                                            ?.call(context, input) ??
                                        input;
                                  },
...
class TIMUIKitChatProviderScope extends StatelessWidget {
  ...
  TIMUIKitChatProviderScope(
    ...
    if (shouldLockInitialUnread) {
      OutgoingVisibleProbe.log(
        'enter_unread_lock_wipe',
        conversationID: conversationID,
        extras: <String, Object?>{'unread': initialUnreadCount},
      );
  ...
  void _lockEntryUnreadWithFrozenSeq({
    ...
    if (isGroup && (firstUnreadSeq == null || firstUnreadSeq <= 0)) {
      unawaited(_refreshLockedFirstUnreadSeq(
        conversationID: conversationID,
        unreadCount: unreadCount,
      ));
    }
    ...
    Future<void>(() async {
      ...
      try {
        ...
        if (!isSearchJump && target == null) {
          ...
          if (!needUnreadSync) {
            ...
            if (globalModel.hasOpenHydrateInFlight(conversationID)) {
              // The app-owned bootstrap may still be filtering the SDK result
              // against the history-clear watermark. Let it keep ownership
              // before starting another LOCAL/CLOUD history request.
              await globalModel.awaitOpenHydrateInFlight(
                conversationID,
        ...
        if (isSearchJump && searchJumpAnchor != null) {
          ...
          if (loaded &&
              messages != null &&
              searchJumpAnchor.isPresentIn(messages)) {
            globalModel.setSearchJumpStatus(
              conversationID,
