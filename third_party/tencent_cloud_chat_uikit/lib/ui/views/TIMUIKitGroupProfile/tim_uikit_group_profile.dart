...
class TIMUIKitGroupProfile extends StatefulWidget {
  ...
  final Function(V2TimGroupMemberFullInfo groupMemberFullInfo,
      TapDownDetails? tapDetails)? onClickUser;

  /// When false, [builder] / default page are not wrapped in [SingleChildScrollView]
  /// (e.g. desktop dual-pane that scrolls each column independently).
    ...
    return MultiProvider(
        ...
        builder: (context, w) {
          ...
          if (needUpdate != null && needUpdate.groupID == widget.groupID) {
            ...
            switch (needUpdate.updateType) {
              case UpdateType.groupInfo:
                if (needUpdate.groupInfoSubType ==
              ...
              case UpdateType.memberEnter:
                model.processGroupMemberListEnter(
                    groupID: widget.groupID, memberList: needUpdate.extraData);
                model.loadGroupInfo(widget.groupID);
                break;
              ...
              case UpdateType.memberListReload:
                model.reloadGroupMembers(widget.groupID);
                model.loadGroupInfo(widget.groupID);
                break;
              default:
                break;
          ...
          Widget groupProfilePage({required Widget child}) {
            final colored = Container(color: pageColor(), child: child);
            if (!widget.scrollable) {
              return colored;
            }
            return SingleChildScrollView(child: colored);
          }
          ...
          void toDefaultNoticePage() {
            ProfileSignatureEditPage.pushGroupNotice(
              context,
              model: model,
              initialNotification: groupInfo.notification ?? '',
            );
          }
                ...
                case GroupProfileWidgetEnum.muteGroupMessageBar:
                  return (customBuilder?.muteGroupMessageBar != null
                      ? customBuilder?.muteGroupMessageBar!(
                          (model.conversation?.recvOpt ?? 0) != 0,
