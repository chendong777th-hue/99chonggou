...
class TIMUIKitChatController {
  ...
  clearHistory([String? convID]) {
    if (convID != null) {
      return globalChatModel.removeMessageList(convID);
    }
  ...
  /// 按系统 viewInsets 重新对齐移动端输入栏（如右滑取消返回）。
  void syncMobileInputLayoutFromViewInsets() {
    textFieldController?.syncLayoutFromViewInsets();
  }

  /// 仅收起表情 / 更多等附属面板（不强制收起系统键盘）。
  void hideAccessoryPanelsOnMobile() {
    textFieldController?.hideAccessoryPanels();
  }
